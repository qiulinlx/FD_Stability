...
def data_preprocessing(df, npp_df, csc_df):
    ...
    fd_df=df.copy()
    fd_df.drop(columns=[ 'Shannon Diversity', "Simpson's Index", "Shannon Equitabiltiy Index", "source_file",  "Functional_Divergences"], inplace=True)
    ...
    fd_df.dropna(subset=['Functional_Evenness'], inplace=True)

    sd_df=df.copy()
    sd_df.drop(columns=['Raos_Q', 'Functional_Evenness', "Functional_Divergences", "Shannon Equitabiltiy Index", "source_file"], inplace=True)
